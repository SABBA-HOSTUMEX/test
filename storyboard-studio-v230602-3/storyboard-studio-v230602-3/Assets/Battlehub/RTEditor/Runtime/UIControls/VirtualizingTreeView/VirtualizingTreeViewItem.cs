...
{ 
    public class VirtualizingTreeViewItem : VirtualizingItemContainer
    {
        ...
        public override object Item
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    //gameObject.SendMessage("ItemBingding", base.Item, SendMessageOptions.DontRequireReceiver);

                    name = base.Item.ToString() + " " + m_treeViewItemData.ToString();

                    /*
                    ProjectItem projectItem = base.Item as ProjectItem;

                    if(projectItem != null)
                    {
                        if (projectItem.Desc != "")
                        {
                            name = name + " - " + projectItem.Desc;


                            Debug.Log(name + " - " + projectItem.Desc);
                        }
                    }
                    */
                    //ItemPresenter.
        ...
        //0:Camera, 1:Light,2:Mesh,3:Scene,4:Char,5:Animation,6:Image



        public void UpdateIconBingding()
        {
            Battlehub.RTCommon.ExposeToEditor gameitem = base.Item as Battlehub.RTCommon.ExposeToEditor;
            ...
            if (gameitem != null)
            {
                ...
                if (gameitem.GetComponent<Camera>() != null)
                {
                    IconKey = 0;
                }
                else if (gameitem.GetComponent<Light>() != null)
                {
                    IconKey = 1;
                }
                else if (gameitem.GetComponent<Animator>() != null)
                {
                    IconKey = 4;
                }
                else if (gameitem.GetComponent<MeshRenderer>() != null)
                {
                    IconKey = 2;
                }
                else if (gameitem.GetComponentInChildren<MeshRenderer>() != null && gameitem.GetComponentInChildren<Camera>() != null)
                {
                    IconKey = 3;
                }
                else if (gameitem.GetComponentInChildren<MeshRenderer>() != null)
                {
                    IconKey = 2;
                }

                if (IconKey != -1 && iconsList.Count > IconKey && iconsList[IconKey] != null) 
                {
                    Image icon = ItemPresenter.GetComponentInChildren<Image>();
                    icon.sprite = iconsList[IconKey];
                }
            }
        }
        ...
        protected override void StartOverride()
        {
            ...
            {
                TMP_Text textUI = ItemPresenter.GetComponentInChildren<TMP_Text>();
                if(TreeView != null && TreeView.ViewDescField == true &&  textUI != null)
                {
                    ...
                    if (projectItem != null)
                    {
                        ...
                        if (projectItem.Desc != "")
                        {
                            textUI.text = textUI.text + "（"+ projectItem.Desc + "）";
                            s_Desc = projectItem.Desc;
                        }
                    }
