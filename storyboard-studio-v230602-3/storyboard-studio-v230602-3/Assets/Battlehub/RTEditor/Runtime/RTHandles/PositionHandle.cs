...
namespace Battlehub.RTHandles
{
    ...
    public class PositionHandle : BaseHandle
    {
        ...
        private HashSet<Transform> m_dragChildTransforms;

        public SceneViewModel SVM;
        ...
        protected override void OnEnable()
        {
            ...
            RuntimeWindow window = Editor.GetWindow(RuntimeWindowType.Scene);
            SVM = window.GetComponentInChildren<SceneViewModel>();
        ...
        protected override void UpdateOverride()
        {
            ...
            if (IsDragging)
            {
                ...
                GameObject go = ActiveTargets[0].gameObject;

                if(false && go.GetComponent<MeshFilter>() != null)
                {
                    if (SVM != null)
                    {
                        ...
                        if (GetPointOnDragPlane(Window.Pointer, out m_prevPoint))
                        {
                            ...
                            SVM.OnObjectDrag(transform.gameObject, m_dragChildTransforms, Window.Pointer, m_prevPoint);
                        }
                    }
                    else
                    {
                        if (GetPointOnDragPlane(Window.Pointer, out m_prevPoint))
                        {
                            ...
                            transform.position = m_prevPoint;
                            Ray DownPointer = new Ray(transform.position, Vector3.down);
                            Ray UpPointer = new Ray(transform.position, Vector3.up);
                            RaycastHit downhit = Physics.RaycastAll(DownPointer).Where(h => !m_dragChildTransforms.Contains(h.transform)).FirstOrDefault();
                            RaycastHit uphit = Physics.RaycastAll(UpPointer).Where(h => !m_dragChildTransforms.Contains(h.transform)).FirstOrDefault();

                            MeshFilter meshFilter;// = GetComponent<MeshFilter>();
                            Bounds bounds;
                            Vector3 fixpoint;
                            if (downhit.transform != null && downhit.transform.gameObject.name != "Colliders")
                            {
                                fixpoint = downhit.point;

                                transform.position = fixpoint;
                            }
                            else if (uphit.transform != null && uphit.transform.gameObject.name != "Colliders")
                            {

                                fixpoint = uphit.point;


                                meshFilter = uphit.transform.gameObject.GetComponent<MeshFilter>();
                                if (meshFilter != null)
                                {
        ...
        protected override bool OnBeginDrag()
        {
            ...
            m_dragChildTransforms =  new HashSet<Transform>(GetComponentsInChildren<Transform>(true));
