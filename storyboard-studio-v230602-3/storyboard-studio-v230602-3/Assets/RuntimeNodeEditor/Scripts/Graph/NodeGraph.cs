...
namespace RuntimeNodeEditor
{
    public class NodeGraph : MonoBehaviour
    {
        ...
        public Node CreateReturn(string prefabPath, Vector2 pos)
        {
            Node node = Utility.CreateNodePrefab<Node>(prefabPath, nodeContainer);
            node.Init(_signalSystem, _signalSystem, pos, NewId(), prefabPath);
            node.Setup();
            nodes.Add(node);
            HandleSocketRegister(node);
        ...
        public void LoadJson(string filestr)
        {
                var graph = JsonUtility.FromJson<GraphData>(filestr);
                ...
                {
                    var nodeData = graph.nodes.FirstOrDefault(data => data.id == node.ID);
                    if(nodeData != null && nodeData.inputSocketIds.Length > 0)
                    {
                        for (int i = 0; i < nodeData.inputSocketIds.Length; i++)
                        {
                            node.Inputs[i].socketId = nodeData.inputSocketIds[i];
