...
namespace UnityGLTF
{
	...
	public partial class GLTFSceneExporter
	{
		...
		private List<Transform> _runtimeanimatedNodes;
...
#region Profiler Markers
		...
		public GLTFSceneExporter(Transform[] rootTransforms, ExportOptions options)
		{
			...
			_runtimeanimatedNodes = new List<Transform>();
	...
	if (nodeTransform.GetComponent<UnityEngine.Animation>() || nodeTransform.GetComponent<UnityEngine.Animator>())
			...
			if (nodeTransform.GetComponent<Battlehub.RTEditor.RuntimeAnimation>() || nodeTransform.GetComponent<UnityEngine.Animator>())
			{

				_runtimeanimatedNodes.Add(nodeTransform);
			}
		...
		private void ExportAnimation()
		{
			for (int i = 0; i < _animatedNodes.Count; ++i)
			{
				...
				ExportRuntimeAnimationFromNode(ref t);
			}

			for (int i = 0; i < _runtimeanimatedNodes.Count; ++i)
			{
				Transform t = _runtimeanimatedNodes[i];
