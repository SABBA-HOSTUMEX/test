...
namespace iiiStoryEditor.UI.ViewModels
{
    ...
    [Binding]
    public class StoryFolderViewModel : HierarchicalDataViewModel<ProjectItem>, IProjectFolderViewModel
    {
        ...
        public virtual void SetItems(ProjectItem[] folders, ProjectItem[] items, bool reload)
        {
            ...
            else
            {
                ...
                if (folders.Length > 0 && folders[0] != null && folders[0].Parent != null)
                {
                    Debug.Log("folders[0].Parent::" + folders[0].Parent.Name);
                    ProjectItem parentFolder = new ProjectItem();
                    parentFolder.Type = "Back";
                    m_items.Insert(0, parentFolder);
                }
        ...
        {
            if (projectItem.Ext == ".rtrun")
            {
                return false;
            }
            if (projectItem.Ext != ".rtprefab")
            {
               // return false;
            }


            if (m_project.Utils.ToType(projectItem) == typeof(GameObject))
            {
                
            }
            //GameObject projGo = projectItem as GameObject;
            //Debug.Log(projectItem.NameExt);
            return true;
        }
