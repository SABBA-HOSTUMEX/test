...
namespace iiiStoryEditor
{
    public class FbxExport : MonoBehaviour
    {

        public string tmpPath = "";
        public int ExportObjects(IEnumerable<UnityEngine.Object> exportObects, string fbxFilePath)
        {
            LastFilePath = fbxFilePath;
            ...
            return ExportAll(exportObects);

        }

        public Texture2D TexToTex2D(Texture mainTexture)
        {
        ...
        const string FileBaseName = "example_fbx";
        ...
        protected void ExportComponents(GameObject unityGo, FbxScene fbxScene, FbxNode fbxParentNode)
        {
            ...
            if (unityAnimator)
            {
                ExportSkinnedMesh(unityGo,  fbxNode, fbxScene, fbxParentNode);
                ExportAnimationClips(unityGo, fbxScene);

            }
            else
            {
                ExportMeshRenderer(unityGo, fbxNode, fbxScene, fbxParentNode);
        ...
        /// </summary>
        public FbxMesh ExportMesh(MeshInfo mesh, FbxNode fbxNode, FbxScene fbxScene)
        {
        ...
        protected void ExportMeshRenderer(GameObject unityGo, FbxNode fbxNode, FbxScene fbxScene, FbxNode fbxParentNode)
        {
            MeshRenderer unityRender  = unityGo.GetComponentInChildren<MeshRenderer>();

            var meshInfo = GetMeshInfo(unityGo);
            if (!meshInfo.IsValid)
                return;

            FbxMesh fbxMesh = ExportMesh(meshInfo, fbxNode, fbxScene);
        ...
        protected void ExportSkinnedMesh(GameObject unityGo, FbxNode fbxNode, FbxScene fbxScene, FbxNode fbxParentNode)
        {
            Animator unityAnimator = unityGo.GetComponent<Animator>();
            if (!unityAnimator)
                return;
            ...
            FbxSurfaceMaterial fbxMaterial, string fbxPropName)
        {
            ...
            Texture2D tex2D = TexToTex2D(unityTexture);

            byte[] bytes = tex2D.EncodeToPNG();
            string texName = unityTexture.name;
            int charAmount = Random.Range(10000, 99999);

            string filename = texName + charAmount.ToString().PadLeft(6, '0');
            string path = tmpPath + "/" + filename + ".png";
            System.IO.File.WriteAllBytes(path, bytes);

            var textureSourceFullPath = path;// AssetDatabase.GetAssetPath(unityTexture);

            Debug.Log("textureSourceFullPath:" + textureSourceFullPath);
        ...
        protected void ExportAnimationClips(GameObject unityRoot, FbxScene fbxScene)
        {
            ...
            Debug.Log("ExportAnimationClips");
        ...
        {
            Renderer renderer = gameObject.gameObject.GetComponent<MeshRenderer>();
            if (requireRenderer)
            {
                // Verify that we are rendering. Otherwise, don't export.
                if (!renderer || !renderer.enabled)
