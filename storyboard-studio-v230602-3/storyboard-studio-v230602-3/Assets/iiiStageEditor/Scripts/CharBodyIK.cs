...
public class CharBodyIK : MonoBehaviour
{
    ...
    {
        var go = AnimationUtility.CreateEffector(name, Vector3.zero, Quaternion.identity);
        return go;
    }

    private static GameObject CreateBodyEffector(string name)
    {
        var go = AnimationUtility.CreateBodyEffector(name, Vector3.zero, Quaternion.identity);
        return go;
    }
    ...
    private GameObject SetupEffector(ref FullBodyIKJob.EffectorHandle handle, string name)
    {
        ...
        if (go != null)
        {
            ...
            if (EffectorRoot != null)
            {
                go.transform.parent = EffectorRoot;
            }
        }
    ...
    private void initSyncIKFromPose()
    {
        m_LeftHandEffector.transform.position = new Vector3(-0.5f,0.78f,0);
    }
    private void SyncIKFromPose()
    {
        ...
        if (m_Animator !=null &&  m_Animator.OpenAnimationStream(ref stream))
        {
    ...
    void OnEnable()
    {
        ...
        if (m_LeftFootEffector == null && GetComponent<Animator>() != null)
        {
            ...
            var clip = AnimationUtility.LoadAnimationClipFromFbx("DefaultMale/Models/DefaultMale_Humanoid", "Idle");
            ...
            m_LeftFootEffector.transform.position = new Vector3(0, 0, 0);
